  ...
  <div>
    ...
    >
      ...
      >
        ...
        <template v-else>
          <!--input输入-->
          <el-form-item label="分享连接">{{ shareUrl }}{{ resultInfo.shareId }} </el-form-item>
          <!--input输入-->
          <el-form-item label="">
            {{ resultInfo.code }}
          </el-form-item>
...
const rules = ref({
  ...
  codeType: [{ required: true, message: "请输入提取码类型" }],
  code: [
    { required: true, message: "请输入提取码" },
    { validate: proxy.Verify.shareCode, message: "请输入数字", trigger: "blur" },
    { min: 5, message: "提取码最少5位数字", trigger: "blur" },
  ],
});
